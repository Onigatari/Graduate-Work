...
json_file = open(f'static/date/rus-eng.txt-model-120.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
model_loaded = model_from_json(loaded_model_json)

model_loaded.load_weights(f'static/date/rus-eng.txt-model-120_weight.h5')
...
with open(f'static/date/rus-eng.txt-tokenizer_input.pkl','rb') as f:
    tokenizer_input = pkl.load(f)
with open(f'static/date/rus-eng.txt-tokenizer_target.pkl','rb') as f:
...
def decode_seq(input_seq):
    ...
    while not stop_condition:
        ...
        if (decoder_word == end_target or len(decoder_sentance) > 70):
            stop_condition = True
